...
export function isCorrect(question: Question, answer: string): boolean {
    let check = answer.toLowerCase();
    check = answer.trim();
    let qcheck = question.expected;
    qcheck = qcheck.toLowerCase();
    qcheck = qcheck.trim();
    if (check.toLowerCase() == qcheck.toLowerCase()) {
        return true;
    } else {
        return false;
    }
...
export function toShortForm(question: Question): string {
    ...
    const firstten = question.name.substring(0, 10);
    return idnum + " " + firstten;
}
...
export function toMarkdown(question: Question): string {
    console.log(question.name.trim());
    const firstline = "# " + question.name;
    console.log(firstline);
    const secondline = question.body;
    if (question.type == "multiple_choice_question") {
        const options = question.options.join("\n");
        const firsreturn = firstline + "\n" + secondline + "\n" + options;
        console.log(firsreturn);
        return firsreturn.trimStart();
    } else {
        console.log(firstline + "\n" + secondline + "\n" + question.options);
        return firstline + "\n" + secondline + "\n" + question.options;
    }
...
export function addOption(question: Question, newOption: string): Question {
    // return the question options question. options = new option
...
//1) produce a new question at the end of everything
//2) set the status of whatever question gets input to false
//3) make the final question have the id and name passed in, and the points provided by the second "question"
